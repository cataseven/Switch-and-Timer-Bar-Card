class SwitchAndTimerBarCard extends HTMLElement {  
  ...
  addEventListeners() {
    this.querySelectorAll('.control-btn.start').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        this._hass.callService('timer', 'start', { entity_id: entity });
      });
    });
    ...
    this.querySelectorAll('.control-btn.stop').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        if (timerEntity) {
          await this._hass.callService('timer', 'finish', { entity_id: timerEntity });
        }
        if (switchEntity) {
  ...
  renderEntity(entityConfig) {
    const { switch: switchEntity, timer: timerEntity, sensor: sensorEntity } = entityConfig;
    ...
    return `
      <div class="entity-card">
        <div class="header">
          <div class="title" style="${switchState?.state === 'on' ? `color: #2196F3` : ''}"> ${entityConfig.name || switchState?.attributes.friendly_name || switchEntity}</div>
        ...
        <div class="control-wrapper ${buttonPosition}">
          ...
          <div class="timeline-container">
            ...
            <div class="time-info">
              ...
              <span>${this.formatTime(activeTime)} / ${this.formatTime(totalDuration)}</span>
            </div>
          </div>
